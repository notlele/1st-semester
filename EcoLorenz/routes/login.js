...
router.post('/', (req, res, next) => {
	...
	try {
		...
		if (isNull(login) || isNull(senha)) {
			...
			Database.query(`SELECT * FROM usuario WHERE CPF = '${login}';`).then(results => {
				...
				if (linhas.length > 0) {
					...
					if (senhaBanco === senha) {
						let user = {
							...
							nome: linhas[0].nmUsuario, // é 'nome' o nome de seu campo?
							login: linhas[0].CPF, // é 'login' o nome de seu campo?
						};
